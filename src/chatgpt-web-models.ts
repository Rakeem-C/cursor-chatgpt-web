...
export type CursorChatGptWebMode = "instant" | "medium" | "high" | "extra-high" | "pro" | "luna";

export const CURSOR_CHATGPT_WEB_DEFAULT_MODE: CursorChatGptWebMode = "high";

export interface ChatGptWebModelRoute {
  slug: string;
  cursorId: string;
  cursorMode: CursorChatGptWebMode;
  displayName: string;
  description: string;
  backendModel: ChatGptWebBackendModel;
...
export const CHATGPT_WEB_MODEL_ROUTES: readonly ChatGptWebModelRoute[] = [
  ...
  },
  {
    ...
    cursorId: "chatgpt-web-high",
    cursorMode: "high",
    displayName: "ChatGPT Web — High",
    description: "ChatGPT Web High through the native Codex harness. Default Cursor specialist.",
    backendModel: CHATGPT_WEB_BACKEND_MODEL,
    codexEffort: "high",
    adapterEffort: "high",
    requiresPro: false,
  },
  {
    ...
    cursorId: "chatgpt-web-extra-high",
    cursorMode: "extra-high",
...
const ALL_CHATGPT_WEB_ROUTES = [CHATGPT_WEB_LUNA_MODEL_ROUTE, ...CHATGPT_WEB_MODEL_ROUTES] as const;

const routesBySlug = new Map(ALL_CHATGPT_WEB_ROUTES.map(route => [route.slug, route]));
const routesByCursorMode = new Map(ALL_CHATGPT_WEB_ROUTES.map(route => [route.cursorMode, route]));

const CURSOR_MODE_ALIASES: Record<string, CursorChatGptWebMode> = {
  "chatgpt-web-instant": "instant",
...
export function parseCursorChatGptWebMode(value: string | undefined | null): CursorChatGptWebMode | undefined {
  if (value == null) return undefined;
  const normalized = value.trim().toLowerCase();
  if (!normalized) return undefined;
  return CURSOR_MODE_ALIASES[normalized];
}

export function isCursorChatGptWebModelId(value: string): boolean {
  return parseCursorChatGptWebMode(value) !== undefined;
}

export function cursorModelIdForMode(mode: CursorChatGptWebMode): string {
  const route = routesByCursorMode.get(mode);
  if (!route) throw new Error(`ChatGPT web cursor mode is not enabled: ${mode}`);
  return route.cursorId;
}

export function requireCursorChatGptWebRoute(
  idOrMode: string | undefined,
  capabilities: ChatGptWebAccountCapabilities,
): ChatGptWebModelRoute {
  const raw = (idOrMode ?? "").trim() || CURSOR_CHATGPT_WEB_DEFAULT_MODE;
  const mode = parseCursorChatGptWebMode(raw);
  if (!mode) throw new Error(`ChatGPT web model is not enabled: ${raw}`);
  const route = routesByCursorMode.get(mode);
  if (!route) throw new Error(`ChatGPT web model is not enabled: ${raw}`);
  return requireChatGptWebModelRoute(route.slug, capabilities);
}

export function availableCursorChatGptWebRoutes(
  capabilities: ChatGptWebAccountCapabilities,
): readonly ChatGptWebModelRoute[] {
  return availableChatGptWebModelRoutes(capabilities);
}
