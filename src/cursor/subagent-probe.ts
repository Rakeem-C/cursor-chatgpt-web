...
export function printCursorSubagentProbeReport(): string {
  ...
  return [
    ...
    "Task(subagent_type=chatgpt-web) is an optional policy wrapper; the child still calls MCP and is not Sol High.",
    "If the child is pinned to Grok, Claude, Composer, or another built-in model, treat native Task as unsupported.",
    "Do not set Override OpenAI Base URL. The picker is experimental and not required for V2.",
    "",
