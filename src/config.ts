...
export function getConfigDir(): string {
  const configured = process.env.CURSOR_CHATGPT_WEB_HOME?.trim() || process.env.CODEX_CHATGPT_WEB_HOME?.trim();
  if (configured) return resolve(expandUserPath(configured));
  const cursorHome = resolve(join(homedir(), ".cursor-chatgpt-web"));
  const legacyHome = resolve(join(homedir(), ".codex-chatgpt-web"));
  if (existsSync(cursorHome) || !existsSync(legacyHome)) return cursorHome;
  return legacyHome;
}
